...
int * MTCNN::faceDetect(unsigned char *imageData, 
    int imageWidth, int imageHeight, int imageChannel, 
    int scale_w, int scale_h)
{
    ...
    ncnn::Mat ncnn_img;
    if (imageChannel == 3) {
        ncnn_img = ncnn::Mat::from_pixels(imageData, ncnn::Mat::PIXEL_BGR2RGB,
                                          imageWidth, imageHeight);
    } else if(imageChannel == 4){
        ncnn_img = ncnn::Mat::from_pixels_resize(imageData, ncnn::Mat::PIXEL_RGBA2RGB, imageWidth,\
                                          imageHeight, imageWidth/scale_w, imageHeight/scale_h);
    }else{
        return nullptr;
    }
    ...
    for (int i = 0; i < num_face; i++) {
        faceInfo[4 * i + 1] = finalBbox[i].x1  *scale_w;//left
        faceInfo[4 * i + 2] = finalBbox[i].y1  *scale_h;//top
        faceInfo[4 * i + 3] = finalBbox[i].x2  *scale_w;//right
        faceInfo[4 * i + 4] = finalBbox[i].y2  *scale_h;//bottom
    }
